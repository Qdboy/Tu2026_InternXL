...
serve(async (req) => {
  ...
  try {
    ...
    // Validate it's a real web URL, not a local file path
    let formattedUrl = url.trim();
    if (formattedUrl.startsWith("file://") || /^[A-Z]:\\/i.test(formattedUrl)) {
      return new Response(JSON.stringify({ error: "Please paste a web URL (https://...), not a local file path. Use the upload button for local files." }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
