...
const Feed = () => {
  ...
  const { profile, location } = useUserLocation();
  const { toast } = useToast();
  const lastAutoGeneratedLocation = useRef<string | null>(null);

  const locationKey = location.residential
    ? `${location.residential.lat.toFixed(4)}:${location.residential.lng.toFixed(4)}`
    : null;
  ...
  const generateArticles = useCallback(
    async (ignoreExistingTitles = false) => {
      if (!profile || generating) return;
      setGenerating(true);
      try {
        const existingTitles = ignoreExistingTitles ? [] : feedItems.map((i) => i.title);
        ...
        if (data?.items) {
          ...
          toast({
            title: "Feed updated",
            description: `${data.items.length} new articles for your location.`,
          });
        }
      } catch (e: any) {
        console.error("Generate feed error:", e);
        toast({
          title: "Couldn't generate articles",
    ...
    [feedItems, generating, profile, toast]
  );
  ...
  useEffect(() => {
    if (!profile || !locationKey || generating) return;
    if (lastAutoGeneratedLocation.current === locationKey) return;

    lastAutoGeneratedLocation.current = locationKey;
    void generateArticles(true);
  }, [generateArticles, generating, locationKey, profile]);
      ...
      const { data, error } = await supabase.functions.invoke("personalize-feed", {
        body: {
          feedItem: {
            ...
            level: item.level,
            relevantOffice: item.relevant_office,
          },
          profile,
        },
      });
  ...
  const filtered =
    activeFilter === "all" ? feedItems : feedItems.filter((item) => item.level === activeFilter);
      ...
      <div className="bg-dark-surface px-5 pt-4 pb-0">
        <div className="flex items-center justify-between mb-3.5">
          <div className="flex items-center gap-2.5">
            ...
            <h1 className="font-display font-black text-on-dark text-xl">
        ...
        <div className="text-center py-20 px-4">
          ...
          {profile && (
            <button
              onClick={() => void generateArticles()}
              disabled={generating}
              className="mt-3 px-4 py-2 bg-primary text-primary-foreground rounded-xl text-sm font-bold"
            >
          ...
          <Card className="rounded-2xl overflow-hidden shadow-md border-primary/15">
            <div className="h-40 relative overflow-hidden">
              ...
              <div className="absolute top-3 left-3">
                <span className="py-1 px-2.5 rounded-full text-[9px] font-extrabold bg-primary text-primary-foreground uppercase tracking-wider">
                  Featured
                </span>
              </div>
              <div className="absolute bottom-3 left-3 right-3">
                <h3 className="font-display text-[15px] font-bold text-on-dark leading-tight">
                  Why Your Local Election Matters More Than You Think
                </h3>
            ...
            <div className="p-3.5">
              <p className="text-[11px] text-muted-foreground leading-relaxed mb-2.5">
                From zoning decisions that shape your neighborhood to school board votes that affect your
                children&apos;s education, local elections have the most direct impact on your daily life —
                yet they see the lowest turnout.
              </p>
              <div className="flex flex-wrap gap-1.5 mb-2.5">
                <span className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-orange-pale text-burnt">
                  Civic Engagement
                </span>
                <span className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-mint text-forest">
                  Local Government
                </span>
              </div>
              ...
              <Card className="rounded-2xl overflow-hidden shadow-sm border-border">
                <div className={`h-32 relative overflow-hidden flex items-end p-3 ${levelGradient[item.level as Level]}`}>
                  <div
                    ...
                    style={{
                      backgroundImage:
                        "repeating-linear-gradient(55deg, rgba(255,255,255,0.03) 0px, rgba(255,255,255,0.03) 1px, transparent 1px, transparent 12px)",
                    }}
                  />
                  <LevelTag level={item.level as Level} />
                </div>
                <div className="p-3.5">
                  <h3 className="font-display font-bold text-foreground leading-tight text-[15px] mb-1.5">
                    {item.title}
                  </h3>
                  ...
                  {item.tags && item.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-2">
                      {item.tags.map((tag) => (
                        <span
                          key={tag}
                          className={`px-2 py-0.5 rounded-lg text-[10px] font-bold ${tagColors[tag] || "bg-secondary text-secondary-foreground"}`}
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  {expandedId === item.id && (
                    <motion.div
                      ...
                      className="rounded-xl bg-orange-pale border border-primary/10 p-3 mt-2"
                    >
                      {loadingIds.has(item.id) ? (
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          <Loader2 className="w-4 h-4 animate-spin" /> Personalizing for you...
                        </div>
                      ) : aiSummaries[item.id] ? (
                ...
                <div className="flex items-center gap-3.5 px-3.5 py-2.5 border-t border-border">
                  ...
                  <button className="flex items-center gap-1.5 text-[11px] font-semibold text-sage ml-auto">
                    <Bookmark className="w-3.5 h-3.5" /> Save
                  </button>
                </div>
              </Card>
